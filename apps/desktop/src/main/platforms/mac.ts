...
export class MacPlatform extends Platform {
    ...
    try {
      ...
      for (const name of lines) {
        try {
          ...
          const { stdout: status } = await this.execute(`networksetup -getinfo "${name}"`);
          const isActive = !status.includes('manually') && status.includes('IP address:');
          
          interfaces.push({
            name,
            displayName: name,
            type: name.toLowerCase().includes('wi-fi') ? 'wifi' 
                : name.toLowerCase().includes('ethernet') ? 'ethernet' 
                : 'other',
            isActive,
          });
        } catch {
  ...
  async flushDnsCache(): Promise<DnsOperationResult> {
    try {
      await this.executeElevated('dscacheutil -flushcache; killall -HUP mDNSResponder');
